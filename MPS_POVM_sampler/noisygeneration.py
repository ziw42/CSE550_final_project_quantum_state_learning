...
try:
    ...
    from evaluation.readMPS import readMPS
except ImportError:
    # Fallback: add project root and evaluation to path
    import os
...
class PaMPS():
    ...
    def samples(self, Ns=1000000, fname='train.txt', N_sample=None):
        # If N_sample is not specified, sample the full chain
        if N_sample is None:
            N_sample = self.N
        if N_sample < 1 or N_sample > self.N:
            raise ValueError("N_sample must be between 1 and N (inclusive). Got N_sample=%d, N=%d" % (N_sample, self.N))
        
        f = open(fname, 'w')
        f2 = open("data.txt", 'w')

        state = np.zeros((N_sample),dtype=np.uint8);
        for  ii in range(Ns):
            
            Pi = np.real(self.l_P[0])  # right environment already includes sites 1..N-1
            Pnum = Pi
            #print Pi,np.sum(Pi)
            ...
            # Sequentially sample sites 1..(N_sample-1), using right environments l_P[i]
            for i in range(1, N_sample):
                ...
                Pden = Pnum[state[i]];
                # Prepare PP for next site if we haven't reached N_sample yet
                if i < N_sample - 1:
                    PP =  ncon((PP,self.LocxM[:,:,state[i]] ,self.MPS[i],self.MPS[i] ),([1,2],[3,4],[1,3,-1],[2,4,-2]))

            one_hot = np.squeeze(np.reshape(np.eye(self.K)[state],[1,N_sample*self.K] ).astype(np.uint8)).tolist()
            print( state)
            #print one_hot
            for item in one_hot:
                f.write("%s " % item)  
...
def main():
    ...
    # Optional 6th argument: N_sample (number of sites to sample)
    N_sample = None
    if len(sys.argv) >= 7:
        N_sample = int(sys.argv[6])
    sampler.samples(Ns=int(sys.argv[5]), N_sample=N_sample)
