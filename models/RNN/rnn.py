...
class LatentAttention():
    def __init__(self, data, K=4, Number_qubits=6 , latent_rep_size=100, gru_hidden=100,decoder='TimeDistributed_mol', Nsamples=0, noise_p=0.0, learning_rate=0.001, epochs=50):
        ...
        # Optionally corrupt training data with multinomial noise at rate p
        # Data is one-hot per site, flattened length L*K
        if self.noise_p > 0.0:
            # reshape to [n_samples, L, K]
            L = Number_qubits
            Kc = K
            dt = self.data_train.reshape([-1, L, Kc])
            # for each site, with prob p, replace one-hot with random other class
            rng = np.random.RandomState(42)
            mask = rng.rand(dt.shape[0], dt.shape[1]) < self.noise_p
            if np.any(mask):
                # current class indices
                cur_idx = dt.argmax(axis=2)
                # sample random indices in [0, K-1]
                rand_idx = rng.randint(0, Kc, size=cur_idx.shape)
                # ensure change: if equal, shift by 1 mod K
                same = rand_idx == cur_idx
                rand_idx[same] = (rand_idx[same] + 1) % Kc
                # apply only where mask
                for i in range(dt.shape[0]):
                    for t in range(dt.shape[1]):
                        if mask[i, t]:
                            one = np.zeros((Kc,), dtype=dt.dtype)
                            one[rand_idx[i, t]] = 1.0
                            dt[i, t, :] = one
            self.data_train = dt.reshape([dt.shape[0], L * Kc])
        ...
        self.checkpoint_dir = os.path.join(
            "RNN_parameters",
            f"L{self.max_length}_K{self.charset_length}_latent{self.latent_rep_size}_hid{self.gru_hidden}_p{self.noise_p}"
        )
        ...
        self.generated_molecules, self.generated_logits = self.generation(decoder, self.molecules)  # training branch: probs and logits

        self.sample_onehot, self.logP = self.generation(decoder)
        self.sample_RNN = tf.argmax(self.sample_onehot, axis=2)
        
        # Use stable softmax-cross-entropy on logits
        per_step_ce = tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.molecules,
                                     logits=self.generated_logits)
        self.generation_loss = tf.reduce_sum(per_step_ce, axis=1)
    ...
    def generation(self,decoder, molecules=None):
        ...
        if decoder == 'TimeDistributed':
            ...
            with tf.variable_scope("generation"):
                ...
                # applying a fully connected layer on top to each time output separately
                in_dense = tf.reshape(outputs, [self.batchsize*self.max_length, self.gru_hidden])
                logits = tf.layers.dense(in_dense, units=self.charset_length, activation=None, name="output")
                probs = tf.nn.softmax(logits)
                h2 = tf.reshape(probs, [self.batchsize, self.max_length, self.charset_length])
                hlogits = tf.reshape(logits, [self.batchsize, self.max_length, self.charset_length])
...
#
#
        # When molecules is provided (training), return both probs and logits for stable loss computation.
        # (For sampling/generation, return probs and logP.)
...
#
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            if nstepspe <= 0:
                raise ValueError(
                    f"Not enough samples ({self.n_samples}) for batchsize={self.batchsize}. "
                    f"Reduce batchsize or provide more data."
                )

            rlist = range(nstepspe)
            lastNumber = nstepspe - 1
            ...
            for epoch in range(self.epochs):
                for idx in range(nstepspe):
                    ...
                    if idx ==  lastNumber:
                        ...
                        saver.save(sess, os.path.join(self.checkpoint_dir, "model"),global_step=epoch)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Standalone RNN trainer (deprecated; use models/RNN/train_rnn.py)')
    ...
    model = LatentAttention(data=args.data, K=args.K, Number_qubits=args.L, latent_rep_size=args.latent, gru_hidden=args.hidden, decoder='TimeDistributed_mol', Nsamples=args.samples, noise_p=args.p, learning_rate=args.lr, epochs=args.epochs)
    model.train()
